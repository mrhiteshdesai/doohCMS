...
import { X, MapPin, Tag, Monitor, RotateCw, Smartphone, Loader } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[80vh] flex flex-col relative overflow-hidden">
        ...
        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
          <h2 className="text-xl font-bold text-gray-800">Add New Screen</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="p-6 overflow-y-auto border-r border-gray-100">
            ...
            <form id="add-screen-form" onSubmit={handleSubmit} className="space-y-5">
              {/* Pairing Code & Name */}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      name="code"
                      placeholder="123456"
                      className="w-full pl-10 pr-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-lg tracking-widest font-mono"
                      value={formData.code}
                      onChange={handleChange}
                      required
                    />
                  </div>
                  <p className="text-xs text-gray-500 mt-1">Enter the 6-digit code displayed on your screen</p>
                </div>
...
const MapEnabledForm = ({ 
  ...
  useEffect(() => {
      ...
      // This event is fired by the Google Maps script when the API key is invalid or billing is not enabled
  ...
  // Extra safety: ensure the Places library is actually available on the window object
  // Sometimes isLoaded is true but the library failed to initialize due to API restrictions
  const placesLibraryLoaded = isLoaded && (window as any).google && (window as any).google.maps && (window as any).google.maps.places;

  const autocompleteRef = useRef<google.maps.places.Autocomplete | null>(null);
  const [map, setMap] = useState<google.maps.Map | null>(null);
  // Default center: Smartags Private Limited
...
const MapDisabledForm = ({ formData, customMessage, ...uiProps }: any) => {
  return (
    <ScreenFormUI
      {...uiProps}
      formData={formData}
...
const AddScreenModal = ({ isOpen, onClose, onSuccess }: AddScreenModalProps) => {
  ...
  if (googleMapsApiKey) {
    return <MapEnabledForm apiKey={googleMapsApiKey} setFormData={setFormData} {...commonProps} />;
  }

  return <MapDisabledForm {...commonProps} />;
};
